...
read_csv("data-srdb/srdb-data.csv", show_col_types = FALSE) %>% 
    select(Study_number, Author, Study_midyear, YearsOfData, 
           ...
           SRDB_ecosystem_type = Ecosystem_type,
    ...
    filter(!SRDB_ecosystem_type %in% c("Agriculture", "Bare", "Urban")) %>% 
...
We want to use the ECMWF land cover codes (`tvh` -- type of high vegetation)
for modeling, because they're wall-to-wall global,
but also to take advantage of the 'local knowledge'
of the SRBD `Ecosystem_type` information. So we overwrite the former
with the latter in the joined dataset.

```{r ecosystem-type}
#| fig-width: 8
#| fig-height: 6

era5_highveg_names <- read_csv("data-ERA5/ECMWF_land_cover_codes.csv", col_types = "ic")
srdb_ecosystem_map <- read_csv("srdb-ecosystems-to-ECMWF.csv", col_types = "ccc")

srdb_sample %>% 
    left_join(era5_highveg_names, 
              by = c("Veg_type_hi" = "Number")) %>%
    left_join(srdb_ecosystem_map,
              by = c("SRDB_ecosystem_type" = "SRDB_ecosystem_type",
                     "SRDB_leaf_habit" = "SRDB_leaf_habit")) %>% 
    # the two cases we're going to overwrite the ECMWF category 
    # are when it's (1) NA or (2) Crops, mixed farming 
...
ggplot(srdb_sample_ecosystems, 
      aes(SRDB_ecosystem_type, Mapped_ECMWF_category, color = SRDB_leaf_habit)) + 
    geom_jitter() +
    ggtitle("How SRDB Ecosystem_type maps to ECMWF category")

ggplot(srdb_sample_ecosystems, 
      aes(ECMWF_category, Mapped_ECMWF_category_final, color = SRDB_leaf_habit)) + 
    geom_jitter() +
    theme(axis.text.x = element_text(angle = 90)) +
    ggtitle("How ECMWF categories get replaced by SRDB-derived categories")
...
srdb_sample_ecosystems %>% 
    select(-starts_with("SRDB_"), -ECMWF_category,
           -Mapped_ECMWF_category, -Veg_type_hi) %>% 
    rename(Ecosystem_type = Mapped_ECMWF_category_final) ->
    srdb_sample
```
