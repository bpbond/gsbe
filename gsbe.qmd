...
dat_filtered %>% 
    mutate(log_Rs_annual = log(Rs_annual),
           log_Rh_annual = log(Rh_annual),
           log_Rs_growingseason = log(Rs_growingseason)) ->
    dat_filtered
...
dat_filtered %>% 
    select(Biome, Ecosystem_type, log_Rs_annual, log_Rh_annual, log_Rs_growingseason,
           Tair, Tsoil_lev1, VWC_lev1, Precip, LAI_high, 
           MODIS_NPP, MODIS_GPP) %>% 
    pivot_longer(Tair:MODIS_GPP, names_to = "covariate_name", values_to = "covariate_value") %>%
    pivot_longer(log_Rs_annual:log_Rs_growingseason) %>% 
...
dat_filtered %>% 
    select(log_Rs_growingseason, log_Rs_annual, log_Rh_annual) %>% 
