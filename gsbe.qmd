...
dat %>% 
    ...
    # most SRDB records are from forests (~3000), then grasslands (~760), ag (~540),
    # shrubland (~230), wetland (~210), and desert (~80). Add the next one, 
    # savanna (~30), because it might be particularly vulnerable to a Birch effect, 
    # but group everything else into "Other"
...
## Test linear model: post-drought PBE effect
...
dat_filtered %>% 
    ...
    select(sqrt_Rs_annual, sqrt_Rs_growingseason, sqrt_Rh_annual,
           Ecosystem_type, SPEI12, PBE, 
           Tair:LAI_high, Soil_type_name) ->
    dat_linear_base

dat_linear_base %>%
    select(-sqrt_Rs_growingseason, -sqrt_Rh_annual) %>% 
    filter(complete.cases(.)) -> 
    dat_linear

dat_linear_base %>%
    select(-sqrt_Rs_annual, -sqrt_Rh_annual) %>% 
    filter(complete.cases(.)) -> 
    dat_linear_gs

dat_linear_base %>% 
    select(-sqrt_Rs_annual, -sqrt_Rs_growingseason) %>% 
    filter(complete.cases(.)) ->
    dat_linear_rh
...
message("Fitting Rs_growingseason linear model with PBE (SPEI12, -1.0) effect...")
m_Rs_growingseason <- lm(sqrt_Rs_growingseason ~ (Tair + Tsoil_lev1 + Tsoil_lev2 +
            VWC_lev1 + I(VWC_lev1 ^ 2) + VWC_lev2 ^ 2 + I(VWC_lev2 ^ 2)) * PBE +
            Ecosystem_type,
        data = dat_linear_gs)
m_Rs_growingseason_reduced <- MASS::stepAIC(m_Rs_growingseason, trace = FALSE)
summary(m_Rs_growingseason_reduced)
par(mfrow = c(2, 2))
plot(m_Rs_growingseason_reduced)
car::Anova(m_Rs_growingseason_reduced, type = "III")

message("Fitting Rh_annual linear model with PBE (SPEI12, -1.0) effect...")
m_Rh <- lm(sqrt_Rh_annual ~ (Tair + Tsoil_lev1 + Tsoil_lev2 +
            VWC_lev1 + I(VWC_lev1 ^ 2) + VWC_lev2 ^ 2 + I(VWC_lev2 ^ 2)) * PBE +
            Ecosystem_type,
        data = dat_linear_rh)
m_Rh_reduced <- MASS::stepAIC(m_Rh, trace = FALSE)
summary(m_Rh_reduced)
par(mfrow = c(2, 2))
plot(m_Rh_reduced)
car::Anova(m_Rh_reduced, type = "III")
...
library(broom)
m_Rs_reduced_tidy <- tidy(m_Rs_reduced) %>% mutate(model = "Rs_annual")
m_Rs_growingseason_reduced_tidy <- tidy(m_Rs_growingseason_reduced) %>% mutate(model = "Rs_growingseason")
m_Rh_reduced_tidy <- tidy(m_Rh_reduced) %>% mutate(model = "Rh_annual")
bind_rows(m_Rs_reduced_tidy, m_Rs_growingseason_reduced_tidy, m_Rh_reduced_tidy) %>% 
...
PBE_interactions %>% 
    mutate(signif = p.value < 0.05, 
           effect = case_when(signif & estimate < 0 ~ "Positive",
                              signif & estimate >= 0 ~ "Negative",
                              .default = "")) %>% 
...
**Conclusion: PBE (potential Birch effect, i.e. having a strong drought the year before a well-watered year) has a strong statistical effect on the temperature and moisture drivers of soil respiration. But, it's inconsistent—there's no clean 'Birch effect.'**
