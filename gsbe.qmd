...
fit_and_test_rf <- function(x_train, x_val) {
    ...
    return(list(training_r2 = rf$r.squared, 
                vars = importance(rf), 
...
fit_and_test_rf(test[groups != 1,], x_val = test[groups == 1,])
...
do_k_fold <- function(x, k = 10) {
    ...
    for(i in seq_len(k)) {
        x_val <- x[groups == i,]
        x_train <- x[groups != i,]
        message("\tk-fold ", i, ": training has ", nrow(x_train), 
                "; validation has ", nrow(x_val))

        rf_out <- fit_and_test_rf(x_val, x_train)
...
do_full_analysis <- function(x, iv_set, predictors, spei_cutoffs) {
    ...
    for(iv in iv_set) {
        ...
        for(spei_cut in spei_cutoffs) {
            ...
            k_fold_out <- do_k_fold(this_x)
            k_fold_out$spei_cut <- spei_cut
            k_fold_out$iv = iv
            results[[paste(iv, spei_cut)]] <- k_fold_out
        }
    }
    bind_rows(results)
}
