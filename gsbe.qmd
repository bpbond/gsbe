...
We use the `neuralnet` package for this. I experimented with the
hidden layer topology and threshold settings to find a configuration that
...
**BUT there's a problem with the above.** Fit a model with _random_ 
previous-year SPEI data:
...
For this we compute a "potential Birch Effect" flag `PBE`: TRUE when
(i) the current year has SPEI >=0 (non-drought) and 
(ii) the previous year had SPEI <= 1 (drought).
...
Null test: is previous-year SPEI significant if conditions were similar, 
i.e. no drought the previous year?
