...
# x = data; f = function to call; k = number of folds
do_k_fold <- function(x, f, k = 10, quiet = TRUE) {
...
message("The test complete-cases dataset has ", dim(dat_base_test)[1],
        " rows and ", dim(dat_base_test)[2], " columns")
message("Full-data model performance:")
rf_all_data <- ranger(sqrt_Rs_annual ~ ., data = dat_base_test, importance = "impurity")
print(rf_all_data)

imps <- sort(rf_all_data$variable.importance, decreasing = TRUE)

# Top three variables...
pdpVars(data = dat_base_test, 
        fit = rf_all_data, 
...
do_k_fold(dat_base_test, fit_and_test_rf, k = K_FOLD) %>% 
    summary()
...
m_xgb <- xgboost(dat_base_test[-1], dat_base_test[1])
message("Variable importance of XGBoost model:")
as_tibble(xgb.importance(m_xgb))

message("k-fold cross-validation results:")
do_k_fold(dat_base_test, fit_and_test_xgboost, k = K_FOLD) %>% 
    summary()
```
...
# 1. Top predictive variablesPredictive value of SPEI
