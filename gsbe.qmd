...
dat_filtered %>% 
    ...
    select(sqrt_Rs_annual, 
           ...
           Tair, Tsoil_lev1, Tsoil_lev2,
           Precip, VWC_lev1, VWC_lev2, 
           Veg_type_hi, Veg_type_low, LAI_high, 
           Soil_type_number) %>% 
    filter(complete.cases(.)) ->
    dat_linear

message("Fitting Rs_annual linear model with PBE (SPEI12, -1.0) effect...")
# We reuse this 'lm_with_PBE' function below
lm_with_PBE <- function(x) {
    lm(sqrt_Rs_annual ~ (Tair + Tsoil_lev1 + Tsoil_lev2 +
            VWC_lev1 + I(VWC_lev1 ^ 2) + VWC_lev2 ^ 2 + I(VWC_lev2 ^ 2)) * PBE +
            Ecosystem_type_number + LAI_high + Veg_type_hi + Veg_type_low + Soil_type_number,
        data = x)
}
m_Rs <- lm_with_PBE(dat_linear)
...
# d
rows <- 20
meandata <- as_tibble(t(as.data.frame(colMeans(dat_linear[sapply(dat_linear, is.numeric)]))))
meandata <- meandata[rep(1, rows),]
meandata$PBE <- c(rep(FALSE, rows / 2), rep(TRUE, rows / 2))

varimpdata <- list()
for(var in c("Tair", "Tsoil_lev1", "Tsoil_lev2",
             "Precip", "VWC_lev1", "VWC_lev2")) {
    this_meandata <- meandata
    meanval <- colMeans(this_meandata[,var])
    this_meandata[var] <- rep(seq(meanval * 0.9, 
                                  meanval * 1.1, 
                                  length.out = rows / 2), 2)
    varimpdata[[var]] <- tibble(var = var,
                                value = pull(this_meandata[var]),
                                PBE = this_meandata$PBE,
                         sqrt_Rs_annual = predict(m_Rs_reduced, 
                                                  newdata = this_meandata))
}

bind_rows(varimpdata) %>% 
    mutate(var = factor(var, levels = c("Tair", "Tsoil_lev1", "Tsoil_lev2", 
                                        "Precip", "VWC_lev1", "VWC_lev2"))) %>% 
    ggplot(aes(value, sqrt_Rs_annual, color = PBE)) +
    geom_line() +
