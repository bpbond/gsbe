...
m_Rs_no_PBE <- lm(sqrt_Rs_annual ~ (Tair + Tsoil_lev1 + Tsoil_lev2 +
            ...
            Ecosystem_type,
        data = dat_linear)
#m_no_PBE_reduced <- MASS::stepAIC(m_Rs_no_PBE, trace = FALSE)

message("Fitting Rs_growingseason linear model without PBE effect...")
m_Rs_growingseason_no_PBE <- lm(sqrt_Rs_growingseason ~ (Tair + Tsoil_lev1 + Tsoil_lev2 +
            VWC_lev1 + I(VWC_lev1 ^ 2) + VWC_lev2 ^ 2 + I(VWC_lev2 ^ 2)) +
            Ecosystem_type,
        data = dat_linear_gs)
#m_Rs_growingseason_no_PBE_reduced <- MASS::stepAIC(m_Rs_growingseason_no_PBE, trace = FALSE)
...
tibble(model_with_PBE = predict(m_Rs_growingseason),
       model_without_PBE = predict(m_Rs_growingseason_no_PBE),
       PBE = dat_linear_gs$PBE, 
       Ecosystem_type = dat_linear_gs$Ecosystem_type) %>% 
    ggplot(aes(model_without_PBE, model_with_PBE)) + 
    geom_point(aes(color = PBE)) + 
    geom_abline() + 
    facet_wrap(~Ecosystem_type) +
    ggtitle("Linear model: effect of including PBE on sqrt_Rs_growingseason")

```

**Conclusion: PBE (potential Birch effect, i.e. having a strong drought in the previous year) has a strong statistical effect on the temperature and moisture drivers of soil respiration. But, it's inconsistent—there's no clean 'Birch effect.'**
