...
preds <- setdiff(colnames(dat_filtered_base_all), dvs)
kf_out <- list()
for(dv in dvs) {
    # Construct dataset and do the k-fold
    x <- dat_filtered_base_all[c(dv, preds)]
...
do_k_fold(dat_base_test, fit_and_test_xgboost) %>% 
    summary()
```
...
for(dv in dvs) {
    ...
    f <- as.formula(paste(dv, "~", as.character(formula(m_Rs))[3]))
