...
SPEI_CACHE <- "SPEI_data_cache.csv"
if(file.exists(SPEI_CACHE)) {
    message("Reading data from SPEI cache...")
    spei <- read_csv(SPEI_CACHE, col_types = "ddTc")    
} else {
...
cache %>% 
    ...
    summarise(Tair = mean(Tair) - 273.1,
              Tsoil_lev1 = mean(Tsoil_lev1) - 273.1,
              Tsoil_lev2 = mean(Tsoil_lev2) - 273.1,
              VWC_lev1 = mean(VWC_lev1),
              VWC_lev2 = mean(VWC_lev2),
              Precip = sum(Precip) * 1000,
              LAI_high = max(LAI_high),
              .groups = "drop") ->
    cache_summary
